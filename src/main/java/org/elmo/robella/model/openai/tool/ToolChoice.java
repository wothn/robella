...
/**
 * OpenAI Tool选择控制
 * 支持字符串形式："none", "auto", "required", "function"
 */
@Data
...
public class ToolChoice {
     ...
     */
    public static ToolChoice ofText(String type) {
     ...
     */
    public static ToolChoice ofFunction(String functionName) {
        ToolChoice choice = new ToolChoice();
     ...
     * 常用预定义值
     */
    public static final ToolChoice NONE = ToolChoice.ofText("none");
    public static final ToolChoice AUTO = ToolChoice.ofText("auto");
    public static final ToolChoice REQUIRED = ToolChoice.ofText("required");

    /**
     * 检查是否为简单字符串类型
     */
    public boolean isStringType() {
        return "none".equals(type) || "auto".equals(type) || "required".equals(type);
    }
    ...
    public static class ToolChoiceDeserializer extends JsonDeserializer<ToolChoice> {
        ...
        public ToolChoice deserialize(JsonParser p, DeserializationContext ctxt)
            ...
            if (node.isTextual()) {
                // 字符串类型
                return ToolChoice.ofText(node.asText());
