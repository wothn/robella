...
public class OpenAIClient implements ApiClient {
    ...
    public Mono<UnifiedChatResponse> chatCompletion(UnifiedChatRequest request, Provider provider) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("[OpenAIClient] chatCompletion start provider={} model={} stream=false", provider.getName(),
        ...
        return webClient.post()
                ...
                .doOnSuccess(resp -> {
                    if (log.isDebugEnabled())
                        log.debug("[OpenAIClient] chatCompletion success provider={} model={}", provider.getName(),
                                finalRequest.getModel());
                })
                .doOnError(err -> log.debug("[OpenAIClient] chatCompletion error provider={} model={} msg={}",
    ...
    public Flux<UnifiedStreamChunk> streamChatCompletion(UnifiedChatRequest request, Provider provider) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("[OpenAIClient] streamChatCompletion start provider={} model={} stream=true", provider.getName(),
                    finalRequest.getModel());
        }
        ...
        return streamTransformer.transform(
                ...
                // ========== 成功完成时记录日志 ==========
                .concatWith(
                        Mono.deferContextual(ctx -> clientRequestLogger.completeStreamRequest(requestId, finalRequest)
                                .onErrorResume(err -> {
                                    log.warn("Failed to complete stream logging for request: {}", requestId, err);
                                    return Mono.empty();
                                })).thenMany(Flux.empty()) // 保持类型 Flux<UnifiedStreamChunk>
                )
                // ========== 发生错误时记录日志并重新抛出错误 ==========
                .onErrorResume(err -> Mono.deferContextual(
                        ctx -> clientRequestLogger.failStreamRequest(requestId, finalRequest, "openai", err)
                                .onErrorResume(e -> {
                                    log.warn("Failed to log stream failure for request: {}", requestId, e);
                                    return Mono.empty();
                                }))
                        .thenMany(Flux.error(err)) // 继续向下游传播原始错误
                );
    }
